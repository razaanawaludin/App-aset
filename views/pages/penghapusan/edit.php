<?php
    $isSelesai = ($status_val === 'selesai');
    $disabledAttr = $isSelesai ? 'disabled' : '';

    PageHeader(
        "Penghapusan Aset",
        $isSelesai ? "Dokumen penghapusan telah selesai" : "Edit dokumen penghapusan aset",
       buttonhref("?pg=$pg&fl=kpenghapusan","Kembali","primary","circle-chevron-left","")
    );

    // Opsi pegawai penanggung jawab
    $opsiPJ = '<option value="" selected disabled>-- Pilih Penanggung Jawab --</option>';
    foreach ($semuaPegawai as $peg) {
        $sel = ($penanggungJawab_val == $peg['IdPegawai']) ? 'selected' : '';
        $opsiPJ .= "<option value=\"{$peg['IdPegawai']}\" $sel>{$peg['Nama']}</option>";
    }

    // Opsi alasan
    $alasanOptions = ['Rusak Berat' => '🔨', 'Hilang' => '🔍', 'Dijual' => '🏷️', 'Hibah' => '🎁', 'Kadaluarsa' => '⏱️'];
    $opsiAlasan = '<option value="" disabled>-- Pilih Alasan --</option>';
    foreach ($alasanOptions as $a => $emoji) {
        $sel = ($alasan_val === $a) ? 'selected' : '';
        $opsiAlasan .= "<option value=\"$a\" $sel>$emoji $a</option>";
    }
    ...
    if (!$isSelesai) {
        ...
        $ak_buttons .= ' ' . button("Btn","Selesai","success","circle-check-big","onclick=\"return confirm('Yakin selesaikan penghapusan ini? Status tidak dapat dikembalikan.')\"");
        $ak_buttons .= ' ' . buttonhref("#","Batal","danger","circle-x","data-bs-toggle=\"modal\" data-bs-target=\"#modalHapus\" onclick=\"konfirmasiHapus('?pg=$pg&fl=edit&aksi=hapus&id=$id')\"");
    }
    // Tombol cetak selalu tampil
    $ak_buttons .= ' ' . buttonhref("#","Cetak","info text-white","printer","onclick=\"cetakBeritaAcara()\"");

    $formAction = "index.php?pg=$pg&fl=edit&id=$id";
?>
    ...
    <div class="row g-4">
                        ...
                        <form method="POST" action="$formAction" autocomplete="off">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="position-relative">
                                    <input type="text" name="no_penghapusan" value="$noPenghapusan_val" class="form-control form-control-lg bg-light border-0 fs-6 ps-5" style="border-radius: 10px;" readonly>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="alasan" class="form-select form-select-lg bg-light border-0 fs-6 text-dark" style="cursor: pointer; border-radius: 10px;" required $disabledAttr>
                                    $opsiAlasan
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="penanggung_jawab" class="form-select form-select-lg bg-light border-0 fs-6 text-dark" style="cursor: pointer; border-radius: 10px;" $disabledAttr>
                                    $opsiPJ
                                </select>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="berita_acara" class="form-control bg-light border-0 fs-6" style="border-radius: 10px; min-height: 150px;" placeholder="Isi berita acara penghapusan aset..." $disabledAttr>$beritaAcara_val</textarea>
        ...
        <div class="col-lg-7">
            ...
            <?php
                PageContentForm(
                    <<<a
                        <form method="POST" action="index.php?pg=$pg&fl=edit&id=$id" autocomplete="off">
                                <label class="form-label small fw-bold text-secondary text-uppercase" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">
                                    <i data-lucide="package" style="width: 14px; margin-bottom: 2px;" class="me-1"></i> Tambah Aset ke Dokumen Penghapusan
                                </label>
                ...
                foreach ($detailAset as $det) {
                    ...
                    if (!$isSelesai) {
                        $btnHapusDetail = <<<hps
                            ...
                            onclick="konfirmasiHapus('?pg=$pg&fl=edit&aksi=hapusbarang&id=$id&idDetail=$idDetail')">
...
row;
                ...
                PageContentTabel(
                    ...
                    $trDetail,
                    <<<ket
                        <span class="text-muted small">Total <strong>$jumlahDetail</strong> aset dalam dokumen penghapusan</span>
                    ket,
                    "")
            ?>
...
<?php
    ...
    $printNoDokumen    = htmlspecialchars($noPenghapusan_val);
    $printNamaPJ       = htmlspecialchars($namaPenanggungJawab);
    $printAlasan       = htmlspecialchars($alasan_val);
...
<body>
    <div class="header">
        ...
        <div class="no-surat">No: <?= $printNoDokumen ?></div>
    </div>

    <div class="info-section">
        <div class="info-row">
            <div class="info-label">Nomor Dokumen</div>
            <div class="info-value">: <?= $printNoDokumen ?></div>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
            <div class="info-label">Alasan Penghapusan</div>
            <div class="info-value">: <?= $printAlasan ?></div>
        </div>
        <div class="info-row">
            <div class="info-label">Penanggung Jawab</div>
            <div class="info-value">: <?= $printNamaPJ ?></div>
        </div>
        <div class="info-row">
    ...
    <div class="ba-title" style="margin-top: 20px;">Daftar Aset yang Dihapus</div>
    <table class="aset">
        <thead>
            <tr>
    ...
    <div class="ttd-section">
        <div class="ttd-box">
            ...
            <div class="ttd-name">Kepala Instansi</div>
        </div>
        <div class="ttd-box">
            <div class="ttd-label">Penanggung Jawab,</div>
            <div class="ttd-line"></div>
            <div class="ttd-name"><?= $printNamaPJ ?></div>
        </div>
    </div>
